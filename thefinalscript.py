...
def nextaction():
    ...
    if len(actionlist) > 0:
        ...
        if currentaction == "wait5":
            ...
            time.sleep(1)
            pyautogui.moveTo(566, 449)
            time.sleep(1)
            pyautogui.click(button='right')
            time.sleep(1)
            #screenimage = ImageGrab.grab() #if the objective is in the same system then dock, this is kinda buggy it seems
            #if (196, 197, 197, 255) == screenimage.getpixel((1169,1081)):
            #    pyautogui.moveTo(587, 535)
            #    pyautogui.click()
            #else:
            pyautogui.moveTo(596, 463)
            pyautogui.moveTo(597, 463)
            pyautogui.click()
            time.sleep(5)
...
def plannextactions():
    if docked == True:
        ...
        if location == "<url=showinfo:1927//60008464 alt='Current Station'>Finid X - Moon 1 - Amarr Navy Assembly Plant</url>":
            ...
            if (121, 157, 72, 255) == screenimage.getpixel((1439,1476)): #agent mission already accepted
        ...
        output('at some random point in space?')
        if checkifrouteset():
            output('finishing mission')
            actionlist.append("warpstart")
            actionlist.append("waituntildocked")
            actionlist.append("startconvo")
            actionlist.append("wait10")
            actionlist.append("completemission")
